	...
	function user_profile(){
		...
		if(isset($_GET['u_id'])){
				...
				echo "<div id='user_profile'>
					...
					<img src='user/user_images/$image' width='75' height='75' />
					<br/>
				...
				$get_friends = "select * from friends where (f1=$id AND f2=$user_id) OR (f1=$user_id AND f2=$id)"; 
				$run_friends = mysqli_query($con,$get_friends);
				$chk=mysqli_num_rows($run_friends);
				if($chk==0){
					echo "
					<a href='addfriend.php?u_id=$id'><button>Add Friend</button></a>";
				}
				else
				{
					echo "
