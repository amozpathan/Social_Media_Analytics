		...
		<!--Header Wrapper ends-->
			...
			<div class="content">
				...
				<div id="content_timeline">
					...
					<?php 
					$myfriends=array();
					array_push($myfriends,$user_id);
					$get_friends = "select f1 from friends where f2=$user_id"; 
					$run_friends = mysqli_query($con,$get_friends);
					
					while($row=mysqli_fetch_array($run_friends)){
					array_push($myfriends,$row[0]);
					$get_members = "select * from users where user_id=$row[0]"; 
					$run_members = mysqli_query($con,$get_members);
					
					while($row2=mysqli_fetch_array($run_members)){
					...
					echo "
					<a href='user_profile.php?u_id=$user'>
					...
					$get_friends = "select f2 from friends where f1=$user_id"; 
					$run_friends = mysqli_query($con,$get_friends);
					...
					?>
					<h2>Friends suggestion:</h2><br/>
					
					<?php 
					$get_members = "select * from users where user_id not in (".implode(",",$myfriends).")"; 
					$run_members = mysqli_query($con,$get_members);
